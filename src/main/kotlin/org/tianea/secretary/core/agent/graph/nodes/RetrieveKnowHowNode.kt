 ...
 * @property adoptedIds 프롬프트에 주입된 노하우의 ID 목록. `touchKnowHow` 노드가 LLM 호출 성공 직후 [KnowHowStore.touch]를
 *   호출할 때 사용한다. 비활성/공집합 경로에서는 비어 있다.
 */
...
private val retrieveLog =
    LoggerFactory.getLogger("org.tianea.secretary.core.agent.graph.nodes.retrieveKnowHow")

/**
 * `retrieveKnowHow` 노드 정의.
 *
 * 입력 텍스트로 코사인 유사도 검색 → 재랭킹 → 토큰 예산 필터링을 거쳐 노하우 블록을 만든다. `touch`는 이 단계에서 호출하지 않는다 — LLM 호출이 실패하면
 * 노하우는 실제로 사용된 것이 아니므로 사용 기록이 남으면 안 된다. 대신 채택된 ID를 캐리어에 실어 [touchKnowHowNodeDelegate]가 callLLM 성공
 * 후에 일괄 갱신한다.
 *
 * `enabled=false`, [ChatContext]에 `chatId`가 없는 경우, 또는 검색·재랭킹 도중 오류가 발생하면 노하우 없이 통과시킨다 — 일시적 인프라 장애가
 * 사용자 응답 전체를 막아서는 안 된다는 원칙.
 *
...
fun retrieveKnowHowNodeDelegate(
    ...
    node("retrieveKnowHow") { input ->
        val passthrough =
            KnowHowWithInput(originalInput = input, knowHowBlock = null, adoptedIds = emptyList())
        if (!enabled) {
            return@node passthrough
        }
        val chatId = currentCoroutineContext()[ChatContext]?.chatId ?: return@node passthrough
