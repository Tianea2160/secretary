 ...
 *
 * 도메인 모델 [KnowHow]와 분리한다 — [KnowHow]는 임베딩을 들고 다니지 않는 순수 도메인 객체이고,
 * 이 엔티티는 pgvector `vector(4096)` 컬럼([embedding])까지 포함하는 영속화 표현이다.
 * 변환은 [KnowHowStore]가 담당한다.
 *
 * [embedding]은 Hibernate `hibernate-vector` 모듈의 [SqlTypes.VECTOR] 매핑으로 pgvector
 * `vector(4096)` 컬럼과 연결된다. 4096차원은 HNSW/IVFFlat 인덱스 한계(≤2000)를 넘으므로
 * 인덱스 없이 풀스캔 검색한다.
 *
 * 테이블 자체는 [KnowHowStore.initSchema]가 `CREATE TABLE IF NOT EXISTS`로 생성하며
 * Hibernate `ddl-auto`에 의존하지 않는다 — `vector` extension을 테이블 생성 전에 보장해야 하기 때문.
 */
@Entity
@Table(name = "know_how")
class KnowHowEntity(
