 ...
 *
 * 영속화는 Spring Data JPA [KnowHowRepository]에 위임하고, 임베딩은 Spring AI [EmbeddingModel] 빈을 직접 호출해 생성한다.
 * 도메인 모델 [KnowHow]와 JPA 엔티티 [KnowHowEntity] 사이의 변환도 이 클래스가 담당한다.
 *
 * 4096차원 벡터는 pgvector HNSW/IVFFlat 한계(≤2000)를 초과하므로 인덱스를 만들지 않고 풀스캔으로 운용한다. 노하우 행 수는 raw 메시지보다 훨씬
 * 적으므로 이 방식이 충분하다.
 *
...
class KnowHowStore(
     ...
     *
     * Hibernate `ddl-auto`가 아니라 명시적 DDL을 쓰는 이유: `embedding` 컬럼의 `vector(4096)` 타입은 `vector`
     * extension이 먼저 설치돼 있어야 생성할 수 있는데, Hibernate 스키마 생성은 extension 설치와의 순서를 보장하지 못한다. 같은 트랜잭션 안에서
     * extension → 테이블 순으로 직접 실행해 순서를 확정한다. `CREATE ... IF NOT EXISTS`이므로 재기동 시 멱등하다.
     *
     * `@PostConstruct`가 아니라 [ApplicationReadyEvent] 리스너인 이유: 네이티브 DDL `executeUpdate`는 활성 트랜잭션을
     * 요구하는데, `@PostConstruct` 시점에는 트랜잭션 프록시가 아직 적용되지 않는다.
     *
     * `CREATE EXTENSION`은 SUPERUSER 권한을 요구할 수 있으므로 실패해도 부팅을 막지 않고 경고만 남긴다 — 운영 환경에서는 DBA가
     * extension을 사전 설치하는 게 일반적이며, 그 경우 본 호출은 no-op이 되어 정상 진행된다.
     *
     * 신규 컬럼([KnowHowEntity.updatedAt])은 `ADD COLUMN IF NOT EXISTS`로 기존 테이블에도 멱등하게 추가되므로
     * `spring.jpa.hibernate.ddl-auto=none` 상태로 안전하게 운용할 수 있다.
     */
     ...
     * 새 노하우를 저장한다.
     *
     * [KnowHow.id]가 비어 있으면 TSID로 자동 발급된다. 임베딩은 [embeddingText]를 기준으로 생성된다 (기본값: [KnowHow.intent]).
     *
     * @param knowHow 저장할 도메인 객체.
     * @param embeddingText 임베딩 생성에 사용할 텍스트. 미지정 시 [KnowHow.intent] 사용.
     * @return 실제 저장된 id (TSID 자동 발급 시 새 값).
     */
    fun save(knowHow: KnowHow, embeddingText: String = knowHow.intent): String {
    ...
    /**
     * 기존 노하우의 intent·body·importance를 갱신한다. 임베딩도 [embeddingText] 기준으로 재생성된다. 해당 id가 없으면 no-op.
     *
     * @param id 갱신할 노하우 id.
     ...
     *
     * 코사인 거리가 작을수록 유사하므로 오름차순 정렬 후 상위 [topK]개를 반환한다. 반환된 [ScoredKnowHow.rerankScore]는 초기값으로
     * similarity 와 동일하다. [rerank] 를 통해 재랭킹하면 갱신된다.
     *
     * @param chatId 사용자 격리 키.
     * @param queryText 임베딩 생성에 사용할 쿼리 문자열.
     * @param topK 반환할 최대 결과 수.
     */
    fun search(chatId: Long, queryText: String, topK: Int): List<ScoredKnowHow> {
        val queryEmbedding = embeddingModel.embed(queryText)
        return repository.searchByEmbedding(chatId, queryEmbedding, PageRequest.of(0, topK)).map {
            row ->
     ...
     * @param topK 반환할 최대 결과 수.
     */
    fun searchSimilar(chatId: Long, intent: String, topK: Int): List<ScoredKnowHow> =
        search(chatId, intent, topK)
     ...
     *
     * 토큰 수는 `body` 문자 수 / 4 로 근사한다 (UTF-8 영어 기준). 이미 [rerank] 로 내림차순 정렬된 리스트를 전제한다.
     *
     * 각 후보를 admit하기 전에 누적 토큰이 예산을 초과하지 않는지 검사하므로, 첫 후보라도 본문이 예산보다 크면 제외된다.
     *
     * @param candidates 재랭킹 완료된 후보 목록 (내림차순).
     * @param tokenBudget 허용 최대 근사 토큰 수.
     * @return 예산 이내 상위 후보.
     */
    fun applyTokenBudget(candidates: List<ScoredKnowHow>, tokenBudget: Int): List<ScoredKnowHow> {
