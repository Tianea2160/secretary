...
private val reflectLog =
    LoggerFactory.getLogger("org.tianea.secretary.core.agent.graph.nodes.reflect")
 ...
 */
data class ReflectOutput(val responseText: String, val candidates: List<KnowHowCandidate>)

/**
 * `reflect` 노드 정의.
 *
 * 이번 턴의 user/assistant 메시지를 [KnowHowReflector]에 넘겨 재사용 가능한 노하우 후보를 추출한다. `enabled=false` 또는
 * [ChatContext] 미존재 시 빈 후보 리스트로 즉시 통과시켜 응답 텍스트만 흘려보낸다.
 *
 * 모든 예외는 격리한다 — reflect 실패가 응답 전달을 막아서는 안 된다. 응답 텍스트는 어떤 경로에서도 그대로 보존된다.
 *
...
fun reflectNodeDelegate(
    ...
    node("reflect") { responseText ->
        ...
        runCatching {
                ...
                val lastUserMsg = messages.lastOrNull { it is Message.User }?.textContent() ?: ""
                val candidates =
