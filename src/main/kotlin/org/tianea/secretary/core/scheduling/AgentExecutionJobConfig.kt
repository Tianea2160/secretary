 ...
 *
 * Spring Boot `QuartzAutoConfiguration`이 이 두 빈을 감지해 `Scheduler`에 자동 등록한다. 따라서
 * `Scheduler.scheduleJob()`을 수동으로 호출하면 JDBC job store에서 `ObjectAlreadyExistsException`이 발생하므로 직접
 * 등록하지 말 것.
 *
...
class AgentExecutionJobConfig {
    ...
    fun agentExecutionJobDetail(): JobDetail =
        JobBuilder.newJob(AgentExecutionJob::class.java)
