 ...
 *
 * **불변식: 모든 노드는 [toLatex]로 원본에 준하는 LaTeX 소스를 재구성한다.** [UnicodeMathRenderer]가 유니코드로 매핑하지 못하는 노드는
 * [toLatex]를 호출해 원본 LaTeX를 그대로 노출한다 (accurate-over-pretty 폴백 정책). 라운드트립은 의미상 동등이면 충분하며 바이트 단위로 일치할
 * 필요는 없다 — 폴백은 변환 *불가능* 노드에서만 의미가 있기 때문이다.
 */
internal sealed interface MathNode {
     ...
     * 노드들의 시퀀스 또는 `{...}` 그룹.
     *
     * @property braced `{...}`로 명시된 그룹이면 true — [toLatex]가 중괄호를 재생성한다. 인자(`\frac`의 분자 등)나 최상위 시퀀스
     *   컨테이너는 false.
     */
    data class Group(val children: List<MathNode>, val braced: Boolean) : MathNode {
    ...
    data class Command(val name: String, val raw: String) : MathNode {
        override fun toLatex(): String = raw
    }

    /** `\frac` / `\dfrac` / `\tfrac`. */
    data class Fraction(val numerator: MathNode, val denominator: MathNode, val kind: String) :
        MathNode {
        override fun toLatex(): String = "\\$kind{${numerator.toLatex()}}{${denominator.toLatex()}}"
    }

    /** `\sqrt{...}` 또는 `\sqrt[index]{...}`. */
    data class Sqrt(val radicand: MathNode, val index: MathNode?) : MathNode {
        override fun toLatex(): String {
    ...
    /** 위/아래 첨자. [base]는 leading script(`^2` 단독) 대비 nullable. `x^2_i`와 `x_i^2`는 같은 노드로 모인다. */
    ...
    data class Accent(val command: String, val base: MathNode) : MathNode {
        override fun toLatex(): String = "\\$command{${base.toLatex()}}"
    }

    /** `\text` `\mathrm` `\mathbf` `\mathbb` `\mathcal` `\mathfrak` 등 폰트/텍스트 래퍼. */
    data class FontWrapper(val command: String, val content: MathNode) : MathNode {
     ...
     */
    data class Delimited(val left: String, val right: String, val content: MathNode) : MathNode {
        override fun toLatex(): String = "\\left$left${content.toLatex()}\\right$right"
    }

    /** `\begin{name} ... \end{name}` 환경. [rows]의 각 원소는 한 행, 각 행의 원소는 한 셀. */
    data class Environment(val name: String, val rows: List<List<MathNode>>) : MathNode {
        override fun toLatex(): String {
            val body =
                rows.joinToString(" \\\\ ") { row -> row.joinToString(" & ") { it.toLatex() } }
            return "\\begin{$name}$body\\end{$name}"
        }
    }

    /** `\sum` `\int` `\prod` 등 큰 연산자. [lower]/[upper]는 `_`/`^`로 흡수한 상하한. */
    data class BigOperator(val command: String, val lower: MathNode?, val upper: MathNode?) :
        MathNode {
        override fun toLatex(): String {
    ...
    data class Unknown(val raw: String, val args: List<MathNode>) : MathNode {
