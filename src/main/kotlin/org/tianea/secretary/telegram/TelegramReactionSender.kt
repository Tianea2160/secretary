 ...
 *
 * agent 실행 시작 시 사용자 메시지에 [PROCESSING_EMOJI] 리액션을 달고, 종료 시(정상·예외 무관) 제거한다.
 * 리액션 호출 실패는 agent 실행을 막지 않도록 삼킨다 — 메시지가 너무 오래됐거나 봇 권한이 없으면
 * Telegram이 거부할 수 있으나, 처리중 표식은 부가 기능이라 본 흐름을 중단시킬 이유가 없다.
 */
@Component
class TelegramReactionSender(
    ...
    fun setProcessing(
        chatId: Long,
        messageId: Int,
    ) = react(chatId, messageId, listOf(ReactionTypeEmoji.builder().emoji(PROCESSING_EMOJI).build()))

    fun clearProcessing(
        chatId: Long,
        messageId: Int,
    ) = react(chatId, messageId, emptyList())
