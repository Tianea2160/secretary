 ...
 * 노출된다. 따라서 마크다운 직렬화 전에 수식 구간을 토크나이저 → 재귀 하강 파서 → 유니코드
 * 렌더러 파이프라인(`org.tianea.secretary.telegram.latex` 패키지)으로 평탄화한다.
 *
 * 변환 불가능한 토큰은 원본 LaTeX를 그대로 노출한다(accurate-over-pretty). 파서가 어떤 입력에서
 * 예외를 던지더라도 해당 `$...$` 구간은 원본 그대로 통과시켜 메시지 전송 자체는 절대 깨지지 않는다.
 *
 * 알려진 한계: `$`가 가격 표기로 쓰인 문장(`$100과 $200`)은 잘못 매칭될 수 있다. 다만 그
 * 구간이 유효 LaTeX가 아니면 폴백으로 원본이 그대로 복원된다.
 */
@Component
class LatexUnicodeRenderer : TelegramRenderer {
    ...
    /**
     * LaTeX 수식 본문 [latex]를 유니코드로 평탄화한다. 파이프라인이 예외를 던지면 구분자까지
     * 포함한 [original]을 그대로 반환한다.
     */
    private fun flatten(
        latex: String,
        original: String,
        ...
        runCatching {
            UnicodeMathRenderer.render(LatexParser(LatexLexer.tokenize(latex)).parse())
