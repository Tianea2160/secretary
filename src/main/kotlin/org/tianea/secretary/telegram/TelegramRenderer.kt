 ...
 *
 * 구현체는 순수 함수처럼 동작해야 한다: 입력 [String]만 보고 동일 출력 [String]을 만들고
 * 외부 상태(I/O, 로깅 외)를 건드리지 않는다. [TelegramMessageSender]는 여러 stage를 순서대로
 * 합성하므로 한 stage가 다음 stage가 이해 못 하는 마커를 남기면 파이프라인이 깨진다.
 *
 * 권장 규약:
